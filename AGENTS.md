...
The kernel (`acp-kernel`) guarantees, and this adapter RELIES on: within a
...
dies (edited/truncated/folded). Summaries cite tags across turns and the model
can cite any number it has ever seen, so a re-issued number silently
misattributes on decompress. Capacity pressure is the kernel's problem to solve
by widening the ref space, never by recycling numbers.

The kernel's ref-slot reclamation (acp-kernel#176, shipped in 0.0.48–0.0.49)
