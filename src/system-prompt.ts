...
If another extension exposes \`subagent_spawn\`, \`subagent_inspect\`, \`subagent_cancel\`, \`subagent_wait\`, or \`subagent_send\`, it is a separate delegation implementation. Use one delegation family per task: use \`acp_delegate\` for the fixed ACP roles, session resume, fleet inspector, and ACP cancellation; use \`subagent_*\` only for tasks explicitly assigned to that extension. Do not start the same task through both families, and do not ask an ACP child to create another subagent tree. ACP children are marked as nested so the pi-subagents implementation rejects recursive spawning.

This session may run acp_delegate tasks in the background. Use acp_delegate_status({ runId }) for a non-blocking progress snapshot with status, elapsed time, and latest activity. Use acp_delegate_wait({ runId }) only to fetch the final result; it BLOCKS until the run finishes or its timeout elapses. Do NOT poll status in a tight loop; a single wait call either returns the result or times out (in which case a completion notification is still injected when the run finishes).
