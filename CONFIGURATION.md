...
| `promptPack` | string | *(unset — identity surface, kernel defaults everywhere)* | 🟢 ACTIVE | Select a named [prompt pack](#prompt-packs) (e.g. the built-in `lean`) — one line instead of a block of inline `promptSections`/`toolPrompts` JSON; applied as the base layer under your inline overrides. |
